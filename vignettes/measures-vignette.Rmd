...
# Motivation of Fairness Measures:

The fairness measures (or metrics) are the foundations of fairness packages. Users can not only use those metrics to diagnose the datasets. We also need those measures to build fairness visualizations so users could detect the fairness problems more easily. Those measures will be further used in bias mitigation algorithms. 
...
Users could use the following codes to create simple fairness measures which only requires one metric.
...
# Equalized Odds
# Users are expected to evaluate Equalized Odds by ratio or absolute difference. A classifier satisfies Equalized Odds if the subjects in the protected and unprotected groups have equal TPR and equal FPR. So we could use the following code to assess Equalized Odds, keep in mind users are expected to add the customized measures to mlr_measures library:
