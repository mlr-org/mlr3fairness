...
In this example, we will use the very famous COMPAS dataset and choose the protected field to be "sex". Then we will show the basic usages of FairnessMeasures in COMPAS dataset. There are three steps we want to demonstrate:
...
## Create the Data Tasks and create the protected field
...
compas = tsk("compas")
paste0("The protected for COMPAS dataset is : ", adult_train$col_roles$pta)

```

We could then see some basic information for compas data task. Keep in mind, users are expected to do some data cleansing process like NULL imputations and data type transformations before create the data tasks.
...
Basic fairness measures could be evaluated using only one metric. Like False Positive Rate Bias and False Negative Rate Bias. From the following example we could see the false positive rate bias for COMPAS ```sex``` is 0.077 and the false negative rate bias for COMPAS ```sex``` is about 0.06.
