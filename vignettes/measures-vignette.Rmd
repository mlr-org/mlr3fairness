...
Some predefined measures are readily available in `mlr3fairness`, but we will also showcase how custom measures can be constrcuted below.
...
also across protected attributes that have more then two classes_
...
**Note**: Depdending on the `operation` we need to set a different `minimize` flag for the measure so subsequent operations based on the measure automatically know if the measure is to be minimized or maximized e.g. during tuning.
...
In the following example we e.g. want to compute the mean of two fairness metrics,
here false negative and true negative rates.
...
and create a new `Measure` that compute the mean (see `aggfun`) of those metrics.
