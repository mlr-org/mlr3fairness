...
If more than one `pta` columns are provided, the hyperparameter
`intersectional` controls whether intersections of protected groups are
formed (e.g. combinations of gender and race). Initialized to `TRUE`. If
`FALSE`, only the group specified by the first element of `pta` is used.

Calls [fairml::frrm](https://rdrr.io/pkg/fairml/man/frrm.html) from
package [fairml](https://CRAN.R-project.org/package=fairml).

## Details

Fair ridge regression learner implemented via package `fairml`. The
'unfairness' parameter has been initialized to 0.05.
...
- [`LearnerRegrFairfrrm$new()`](#method-LearnerRegrFairfrrm-new)

- [`LearnerRegrFairfrrm$clone()`](#method-LearnerRegrFairfrrm-clone)
...
print(learner)
#> 
#> ── <LearnerRegrFairfrrm> (regr.fairfrrm) ───────────────────────────────────────
#> • Model: -
