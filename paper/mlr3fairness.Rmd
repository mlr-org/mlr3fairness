...
Similarly, whether a selected fairness notion is actually fair depends on the societal context in which a decision is made and which action should be derived from a given prediction.
...
Even an *initially fair* model might then lead to adverse effects in the future [@schwobel-facct22a].
...
As the split into training set and test set is stochastic, the procedure should be repeated multiple times for smaller datasets [@bischl2016] and the resulting performance values should be aggregated.
...
Furthermore, custom fairness measures can be easily implemented by composing them from both regular performance metrics and by combining performance metrics with fairness measures.
This process is extensively documented in an accompanying vignette.
