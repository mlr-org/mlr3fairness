...
It contains fairness metrics, visualizations, and model-agnostic pre- and postprocessing operators as well as reporting functionality to allow for fairness audits and conveniently build unbiased models.
In the remainder of the manuscript, we first provide an introduction to fairness in machine learning.
We then briefly introduce the \CRANpkg{mlr3} [@mlr3] package before introducing the functionality
shipped with our package.
...
The goal of **debiasing techniques** is now to reduce such gaps, either by using data pre-processing, model inprocessing or post-processing techniques on the predictions. This can be achieved by changing
instance weights before training [@kamiran2012data], directly learning fair models [@Zafar2017] or subsequent adaption of model predictions [@hart2016equality]. While perfect fairness is often an elusive goal, a more practical goal is often to optimize utility under a maximum un-fairness constraint.
...
Before we present new functionalities in **mlr3fairness**, we provide a short introduction to \CRANpkg{mlr3} to facilitate understanding of the following sections.
...
Table: (\#tab:metrics) Selection of implemented fairness metrics. The full list can be obtained using `mlr_measures_fairness`.
...
We further add a fairness report that creates an automated report using large variety of fairness metrics and visualization.
Functions generate a R Markdown file [@rmarkdown] that can and should be further customized by the user to
