...
In the following, we denote the fairness metric we want to minimize with $\Delta_M$ and a strictly positive performance metric with $\rho$ (assuming the latter should be maximized)
\[
...
```{r predplots, echo = FALSE, fig.height=3, fig.width=6, fig.align='center', fig.cap = "Visualizing predictions of the decision tree model. Left: Prediction densities for the negative class for races Caucasian and African-American. Right: Fairness metrics comparison for FPR, TPR, EOd metrics."}
library(patchwork)
...
\CRANpkg{mlr3fairness} provides several debiasing techniques that can be used together with a `Learner` to obtain fairer models.
...
combined with arbitrary learners using the `%>>%` operator to build a pipeline.
...
```
 ...
 Table: Overview of available debiasing techniques.\label{tab:debiasing}
