...
If biases are detected and need to be mitigated, we furthermore require debiasing techniques that tightly integrate with the fitting and evaluation of the resulting models in order to obtain trade-offs between the fairness and utility (e.g., predictive accuracy).

In this article, we present the \CRANpkg{mlr3fairness} package.
It contains fairness metrics, visualizations, and model-agnostic pre- and postprocessing operators.
Additionally, it comes with reporting functionality to allow for fairness audits and conveniently building unbiased models.
In the remainder of the manuscript, we first provide an introduction to fairness in ML.
We then briefly introduce the \CRANpkg{mlr3} [@mlr3] package which \CRANpkg{mlr3fairness} builds upon before introducing the extensions for fairness.
We conclude with a short case study demonstrating \CRANpkg{mlr3fairness}'s capabilities.
...
With this proliferation, such decisions have become subject to scrutiny as a result of prominent inadequacies or failures of such methods for example in the case of the COMPAS recidivism prediction system [@compas] - heavily influencing decisions about bail or parolle for suspects and offenders .
...
Similarly, unprivileged populations are often not represented in data due to **sampling biases** leading to models that perform well in groups sufficiently represented in the data but worse on others [@buolamwini2018gender] - this includes higher rate of missing data.
...
The answer to this question depends on a society's ethical values and whether we take a neutral or a normative position, thus different metrics have been proposed.
...
For the sake of simplicity, we here consider a *binary classification* scenario and a *binary protected attribute*.
...
A model is now considered fair, iff $|\Delta_{\mathrm{TPR}}| < \epsilon$ for a given threshold $\epsilon > 0$, e.g., $\epsilon = 0.05$ to allow for small deviations from perfect fairness due to variance in the estimation of $\mathrm{TPR}_{A=\star}$ or additional sources of bias.
...
This can be achieved by changing instance weights before training [@kamiran2012data], directly learning fair models [@Zafar2017] or subsequent adaption of model predictions [@hardt2016equality]. While perfect fairness is often an elusive goal, a more practical goal is often to optimize utility under a maximum unfairness constraint.
Since debiasing techniques are often tailored towards a particular fairness metric, the optimal choice
of debiasing technique is often not trivial and a combination of algorithms and debiasing techniques
...
If the labels $Y$ arise from a biased *label generating process*, such as past loan decisions, there is little hope in obtaining fair outcomes even if fairness metrics indicate this.
...
Additionally, we further describe the \CRANpkg{mlr3} package to provide an overview over the ecosystem \CRANpkg{mlr3fairness} is integrated in.
-->

Several R packages provide similar capabilities to our software, but mostly focussing on fairness metrics and visualization.
...
Interoperability with the \pkg{scikit-learn} [@sklearn] ML framework allow for debiasing a wide variety of ML models in all aforementioned systems.
...
Before we present new functionalities in \CRANpkg{mlr3fairness}, we provide a short introduction to \CRANpkg{mlr3} to facilitate understanding of the following sections and examples.
...
\CRANpkg{mlr3} provides the infrastructure to fit, resample, or evaluate over 100 ML algorithms using a unified API.
Multiple extension packages bring numerous additional advantages and functionality.
...
It is a wrapper around an ML algorithm, e.g., around an implementation of a logistic regression or a decision tree.
...
We start our exposition with the COMPAS data set [@compas].
The task is to predict whether a parolee will re-offend within a span of 2 years.
The preprocessed data set is shipped with \CRANpkg{mlr3fairness}, the corresponding task with protected attribute `"sex"` can be loaded via `tsk("compas")`.

For brevity, we proceed with a subset of the COMPAS data set available as `tsk("compas_race_binary")` with `"race"` as new binary protected attribute with levels `"Caucasian"` and `"African American"`.
As an ML algorithm, we again fit a Classification Tree (implemented in \CRANpkg{rpart}) on 2/3 of our example data and predict class probabilities for the left-out observations.

```{r, echo = FALSE, message = FALSE}
task = tsk("compas_race_binary")
print(task)

idx = partition(task, ratio = 2/3)
model = lrn("classif.rpart", predict_type = "prob", cp = 0.001, maxdepth = 12)
model$train(task, row_ids = idx$train)
prediction = model$predict(task$clone(), row_ids = idx$test)
```

With \CRANpkg{mlr3fairness} package loaded, fairness measures can now be constructed like any other measure via `msr()`.
Note that fairness measures are prefixed with *fairness*, and simply calling `msr()` without any arguments will return a list of all available measures.
Table \ref{tab:metrics} gives a brief overview over some popular measures which are readily available.
Other fairness measures can easily be constructed by composing regular performance metrics or by combining performance metrics and fairness measures.

For this example, we choose the binary accuracy measure `"classif.acc"` and the equalized odds metric from above using `"fairness.eod"`.
...
Table: Selection of implemented fairness metrics. The full list can be obtained using `mlr_measures_fairness`.\label{tab:metrics}
...
<!-- FIXME:
?Comment?: do you want to introduce cite fairnes tensor paper
-->
```{r}
...
We further add an automated report template using R markdown [@rmarkdown] which includes many fairness metrics and visualizations to provide a good starting point in order to generate a full fairness report.
