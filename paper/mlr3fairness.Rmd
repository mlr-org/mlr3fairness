...
In this article, we present the package \CRANpkg{mlr3fairness}, which implements a wide variety of metrics that can be used to conduct fairness audits for ML algorithms along with visualizations for model predictions and resulting metrics.
...
<!--
TODO: Bisschen unstrukturiert der Paragraph ... was ist mit debiasing?
-->
...
\mathbb{P}\left(\hat{Y} = 1 \mid A = 0, Y = y\right) = \mathbb{P}\left(\hat{Y} = 1 \mid A = 0, Y = y\right), \quad y \in \{0,1\}.
\end{equation}
<!-- FIXME: Die Formel stimmt so nicht, LHS == RHS -->
...
\mathbb{P}\left(\hat{Y} = 1 \mid A = 0, Y = 1\right) = \mathbb{P}\left(\hat{Y} = 1 \mid A = 0, Y = 1\right).
\end{equation}
<!-- Die Formel stimmt so nicht, LHS == RHS -->
<!-- FIXME: Die Formel stimmt so nicht, LHS == RHS -->
...
It provides a useful abstraction that allows for seamlessly switching between different data formats such as `data.table`s, sparse matrices or databases.
...
A `Learner` is a wrapper around an ML algorithm, e.g. around an implementation of a logistic regression or a decision trees.
...
debiasing on the popular adult data set [@uci]. The goal is to predict whether an
