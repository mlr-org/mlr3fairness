...
This is highlighted by the GDPR requirement to process data fairly.
...
If e.g. societal biases lead to disparate measurements of an observed quantity (e.g. SAT scores) for individuals with the same underlying ability, *separation* based metrics might not identify existing biases.
...
\]
When $\Delta_{\mathrm{M}}$ now significantly deviates from $0$, this can be indicative of a fairness violation with respect to the fairness notion described via $M$.
...
\]
When $\Delta_{\mathrm{FPR}}$ now significantly deviates from $0$, the prediction $\hat{Y}$ violates the requirement for *equality of opportunity* formulated above.
...
Instead, to provide a binary conclusion regarding fairness, a model could be considered fair, if $|\Delta_{\mathrm{TPR}}| < \epsilon$ for a given threshold $\epsilon > 0$, e.g., $\epsilon = 0.05$. This allows for small deviations from perfect fairness due to variance in the estimation of $\mathrm{TPR}_{A=\star}$ or additional sources of bias. 
It is important to note, that choosing apropriate thresholds is difficult and widely used values for $\epsilon$ such as $0.05$ are arbitrary and do not translate to legal doctrines, such as e.g. disparate impact [@chen22].
A more in-depth treatment of metrics along with additional fairness metrics are described in (@saleiro2018aequitas, @kim2020fact, @mehrabi and @wachter-vlr2020).
...
This can help to detect biases, stemming e.g. from data acquisition, but might be problematic in cases where e.g. labels are biased.
...
In this setting, fairness notions are employed with the goal to audit ML models or to select which model should be used in practice.
It is important to note, that fairness metrics should however not be used as the sole basis for making decisions about whether to employ a given ML model or to assess whether a given system is fair.
...
This can be achieved via constraints on the employed fairness measure, e.g. $|\Delta M| \leq \epsilon$ requiring that the absolute difference in a metric $M$ between groups 
is smaller than a chosen value $\epsilon$.
In the following, we denote the fairness metric we want to minimize with $\Delta M$ and the performance metric with $\rho$ (assuming the latter should be maximized). 
\[
...
The result of optimization is the *pareto-set*: A list of models which optimally trade off the specified objectives.
...
A different set of risk is posed by *fairwashing* [@fairwashing], i.e. finding fair explanations or satisfying fairness metrics for otherwise unfair models.
...
\[
  if \; |\Delta_{EOd}| \leq \epsilon: accuracy \;\; else: \;  - |\Delta_{EOd}|
\]
...
Whether the achieved accuracy is sufficient, needs to be determined, e.g. from business context.
